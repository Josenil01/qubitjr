...
function getSupabase() {
  ...
  const key = process.env.SUPABASE_SERVICE_ROLE_KEY;
  if (!url || !key) {
    console.error('❌ SUPABASE_URL or SUPABASE_SERVICE_ROLE_KEY not set. Available env keys:', Object.keys(process.env).filter(k => k.startsWith('SUPABASE')));
    return null;
  }
  _supabase = createClient(url, key);
  console.log('✅ Supabase client created for:', url);
  return _supabase;
}
...
router.post('/query', async (req, res) => {
  const supabase = getSupabase();
  if (!supabase) return res.status(503).json({ error: 'Database not configured', hint: 'SUPABASE_URL and SUPABASE_SERVICE_ROLE_KEY must be set in Vercel environment variables' });
  try {
