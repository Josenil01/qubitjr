...
- OBRIGATÓRIO: quando a "sequência" de um personagem mostrar um bloco com um valor entre colchetes que NÃO seja "say" (ex.: "forward[3]", "wait[10]", "repeat[4]", "setspeed[normal]"), o TEXTO da dica precisa mencionar esse valor explicitamente - NUNCA deixe implícito ("faça o Carro andar pra frente" sem dizer quanto), ou o aluno fica em tentativa e erro sem saber o número certo. "say" é a ÚNICA exceção - a fala pode ser sugerida (ver regra acima) mas o aluno pode escrever qualquer coisa, então não precisa de um número. Guia de como mencionar cada valor no texto: forward/back/up/down/left/right/hop → "ande/vá N passos" (o número entre colchetes é a quantidade de passos); repeat → "repita N vezes"; setspeed → use a palavra que já vem pronta entre colchetes ("lenta"/"normal"/"rápida"), ex. "com uma velocidade normal"; wait → decisão explícita do usuário, use SEMPRE esta forma exata: "Na [Personagem] use o bloco de tempo para fazer esperar N[, e depois ...]" - "bloco de tempo" é o nome que o aluno reconhece pro bloco de espera; NUNCA use a forma antiga "faça o [Personagem] esperar até aparecer o número N no bloco". grow/shrink (sem um jeito natural de nomear a unidade) → "aperte o bloco até aparecer o número N", ex. "aperte o bloco até aparecer o número 5 pra crescer". Exemplos: sequência "forward[3]" → "Faça o Carro andar 3 passos pra frente."; sequência "wait[60] → say[\"Vamos ué!\"]" da Tartaruga → "Na Tartaruga use o bloco de tempo para fazer esperar 60, e depois dizer 'Vamos ué!'.". Não existe um campo separado no JSON pra esse valor (ver "character_missing_block_type" abaixo) - é só o texto que precisa mencionar.
