...
export default class Lobby {
    ...
    static appinit (v) {
        libInit();
        version = v;
        console.log('[Lobby.appinit] Inicializando Lobby...');
        // Verificar se há um lugar definido globalmente (pelo Router)
        var place = window.currentLobbyPlace || getUrlVars().place;
        // Limpar a variável global após usar
        window.currentLobbyPlace = null;
        ScratchAudio.addSound('sounds/', 'tap.wav', ScratchAudio.uiSounds);
        ScratchAudio.addSound('sounds/', 'cut.wav', ScratchAudio.uiSounds);
        ScratchAudio.init();
        console.log('[Lobby.appinit] Chamando setPage(' + (place ? place : 'home') + ')');
        Lobby.setPage(place ? place : 'home');

        if (window.Settings.settingsPageDisabled) {
            const settingsEl = gn('settings');
            if (settingsEl) settingsEl.style.visibility = 'hidden';
        }

        const homeTab = gn('hometab');
        if (homeTab) {
            homeTab.onmousedown = function () {
        ...
        const helpTab = gn('helptab');
        if (helpTab) {
            helpTab.onmousedown = function () {
        ...
        const bookTab = gn('booktab');
        if (bookTab) {
            bookTab.onmousedown = function () {
        ...
        const gearTab = gn('geartab');
        if (gearTab) {
            gearTab.onmousedown = function () {
        ...
        const aboutTab = gn('abouttab');
        if (aboutTab) {
            aboutTab.onmousedown = function () {
        ...
        const interfaceTab = gn('interfacetab');
        if (interfaceTab) {
            interfaceTab.onmousedown = function () {
        ...
        const paintTab = gn('painttab');
        if (paintTab) {
            paintTab.onmousedown = function () {
        ...
        const blocksTab = gn('blockstab');
        if (blocksTab) {
            blocksTab.onmousedown = function () {
        ...
        console.log('[Lobby.appinit] ✅ Inicialização completa');
    }
    ...
    static loadProjects (p) {
        console.log('[Lobby.loadProjects] Carregando projetos...');
        document.onmousemove = undefined;
        gn('topsection').className = 'topsection home';
        const localizedText = Localization.localize('MY_PROJECTS');
        console.log('[Lobby.loadProjects] MY_PROJECTS =', localizedText);
        gn('tabheader').textContent = localizedText;
        gn('subtitle').textContent = '';
        gn('footer').className = 'footer off';
        var wrapc = gn('wrapc');
        wrapc.scrollTop = 0;
        wrapc.className = 'contentwrap scroll';
        console.log('[Lobby.loadProjects] wrapc estado:');
        console.log('  - display:', window.getComputedStyle(wrapc).display);
        console.log('  - visibility:', window.getComputedStyle(wrapc).visibility);
        console.log('  - width:', window.getComputedStyle(wrapc).width);
        console.log('  - height:', window.getComputedStyle(wrapc).height);
        console.log('  - className:', wrapc.className);
        var div = newHTML('div', 'htmlcontents home', p);
        div.setAttribute('id', 'htmlcontents');
        console.log('[Lobby.loadProjects] Chamando Home.init()...');
        Home.init();
        console.log('[Lobby.loadProjects] ✅ Projects loaded');
    }
    ...
    static goHome () {
        if (currentPage === 'home') {
            // Use SPA Router para navegar de volta ao splash
