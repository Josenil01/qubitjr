...
// Always 1.0. A dynamic version of this (shrinking proportionally on short
// viewports) was tried and reverted: the CSS ${N * scaleMultiplier} usages
// are permanently frozen at scale=1 by vite.config.js's build-time
// substitution and can never see a live value, while ~16 files read this
// same constant directly in JS to position/size things that sit inside
// that frozen CSS. Making it dynamic desynced those two, misaligning
// palette blocks and category icons from their never-shrinking
// containers. Revisit only alongside a real fix on the CSS side.
export const scaleMultiplier = 1.0;
...
export function libInit () {
    ...
    // Garantir que libframe existe (cria dinamicamente se não existir)
    ensureEditorFrames();
}

/**
 * Garante que todos os frames necessários existem no DOM
 * Cria dinamicamente se não existirem (para SPA)
 */
export function ensureEditorFrames () {
    // Garantir #libframe para Library (asset picker)
    let libFrame = document.getElementById('libframe');
    if (!libFrame) {
        libFrame = document.createElement('div');
        libFrame.id = 'libframe';
        libFrame.className = 'libframe';
        document.body.appendChild(libFrame);
    }
    
    // Garantir #paintframe para Paint (paint editor)
    let paintFrame = document.getElementById('paintframe');
    if (!paintFrame) {
        paintFrame = document.createElement('div');
        paintFrame.id = 'paintframe';
        paintFrame.className = 'paintframe';
        document.body.appendChild(paintFrame);
    }
}
/**
    ...
    while ((i < len) && ((j = s.indexOf('$', i)) != -1)) { // eslint-disable-line no-cond-assign
        ...
        if ((i < (len - 1)) && (s[i] === '{')) {
            ...
            if (end != -1) {
                ...
                try {
                    var evaluated = eval(expression);  // eslint-disable-line no-eval
                    result += evaluated;
                } catch (err) {
                    result += `$\{${expression}\}`;  // Manter literal se houver erro
                }
                i = end + 1;
            } else {
                result += '$';
