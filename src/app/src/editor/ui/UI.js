...
export default class UI {
    ...
    static topSection () {
        ...
        // Criar o botão flip no topsection POR ÚLTIMO (para ficar no topo do DOM)
        var flip = newHTML('div', 'flipme', div);
    ...
    static quitFullScreen () {
        ...
        stage.setAttribute('class', 'stage normal');
        UI.nextpage.setAttribute('class', 'nextpage off');
        UI.prevpage.setAttribute('class', 'nextpage off');
        
        // Aplicar left explícito baseado na classe 'editable' ou 'noteditable'
        if (stage.classList.contains('editable')) {
            stage.style.left = '92px';
        } else {
            stage.style.left = '26px';
        }
        ...
        // Forçar recalculation de layout com delay maior
        // (necessário porque display: none precisa ser removido primeiro)
        setTimeout(() => {
            UI.refreshLayoutAfterFullscreen();
        }, 150);
    }

    static refreshLayoutAfterFullscreen () {
        ...
        // Usar a mesma lógica que createScripts() usa para calcular altura
        var h = Math.max(getDocumentHeight(), frame.offsetHeight);
        var newHeight = h - scripts.offsetTop;
        
        // Aplicar a nova altura
        scripts.style.height = newHeight + 'px';
        
        // Atualizar scroll se disponível
        if (ScriptsPane.scroll) {
            ScriptsPane.scroll.update();
        }
        
        // Força o browser a recalcular o layout (reflow trigger)
        var _ = scripts.offsetHeight;
        var stageFrame = gn('stageframe');
        if (stageFrame) {
            _ = stageFrame.offsetHeight;
        }
        
        // Re-aplicar o tamanho da janela para recalcular valores css_vh/css_vw
        if (window.dispatchEvent) {
            window.dispatchEvent(new Event('resize'));
        }
        
        // Recalcular valores CSS dinâmicos se disponível
