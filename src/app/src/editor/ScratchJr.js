...
export default class ScratchJr {
    ...
    static flippage () {
        ...
        function doNext () {
            iOS.cleanassets('svg', ScratchJr.goToLobby);
        }
    }
    ...
    static goToLobby () {
        // Navigate back to lobby via SPA Router
        var gotoUrl = ScratchJr.getGotoLink();
        var place = 'home';
        
        if (gotoUrl.indexOf('place=help') > -1) {
            place = 'help';
        }
        
        // Use SPA Router para navegar de volta ao lobby
        window.currentLobbyPlace = place;
